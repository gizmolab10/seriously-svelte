import { k, debug, Angle, svgPaths } from '../common/Global_Imports';
...
export default class Rotation_State {
	...
	get highlight_cursor(): string { return new Angle(this.active_angle!).cursor_forAngle; }
	...
	get cursor(): string {
		const cursor = this.isHighlighted ? this.highlight_cursor : k.cursor_default;
		debug.log_action(` ${cursor} cursor ROTATE`);
		return cursor;
	}
}
