import { G_Widget, G_ArcSlider, S_Paging, S_Rotation, T_Line, T_Widget, T_Quadrant } from '../common/Global_Imports';
	...
	update_widget_angles() {
		...
		if (this.widgets_shown > 0 && !!this.predicate) {
			...
			while (index < this.widgets_shown) {
				...
				const g_widget = new G_Widget(Rect.zero, T_Line.flat, child_ancestry, origin_ofChild, this.points_toChildren, angle_ofChild);
				this.g_cluster_widgets.push(g_widget);
				child_ancestry.g_widget = g_widget;
				index += 1;
			}
