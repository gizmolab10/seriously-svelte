	...
	async things_fetch_all() {
		...
		try {
			...
			for (const remoteThing of remoteThings) {
				...
				h.thing_remember_runtimeCreate(k.empty, id, fields.title as string, fields.color as string, (fields.type as T_Thing) ?? fields.trait as string, k.empty, true, !fields.type);
			}
		} catch (error) {
	...
	async traits_fetch_all() {
		...
		try {
			...
			for (const record of records) {
				...
				h.trait_remember_runtimeCreateUnique(k.empty, id, ownerIDs[0], type, text, k.empty, true);
			}
		} catch (error) {
	...
	async relationships_fetch_all() {
		...
		try {
			...
			for (const record of records) {
				...
				h.relationship_remember_runtimeCreateUnique(k.empty, id, kind, parents[0], children[0], [order, 0], k.empty, T_Create.isFromPersistent);
			}
		} catch (error) {
	...
	async predicates_fetch_all() {
		try {
			...
			for (const record of records) {
				...
				h.predicate_remember_runtimeCreate(id, kind, isBidirectional, k.empty);
			}
