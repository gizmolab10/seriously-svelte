...
export default class Ancestry extends Identifiable {
	...
	g_widget_forGraphMode(t_graph: T_Graph): G_Widget {
	...
	svgPathFor_tinyDots_outsideReveal(points_toChild: boolean): string | null {
		const in_radial_mode = c.inRadialMode;
	...
	get pointsNormal(): boolean {
		...
		return c.inRadialMode ? radial_pointsNormal : !hasVisibleChildren;
	}
	...
	showsReveal_forPointingToChild(points_toChild: boolean): boolean {
		const isRadialFocus = c.inRadialMode && this.isFocus;
