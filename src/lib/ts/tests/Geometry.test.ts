...
describe('Angle', () => {
	...
	it('quadrant', () => {
		expect(new Angle(Math.PI / 3).quadrant_ofAngle).toBe(E_Quadrant.upperRight);	// 60° is in upper right quadrant
	});

	it('orientation', () => {
		expect(new Angle(Math.PI / 3).orientation_ofAngle).toBe(E_Orientation.up);		//	60° orients up
		expect(new Angle(Math.PI / 2).orientation_ofAngle).toBe(E_Orientation.up);		//	90° orients up
		expect(new Angle(Math.PI / 6).orientation_ofAngle).toBe(E_Orientation.right);	//	30° orients right
		expect(new Angle(Math.PI * 1.25).orientation_ofAngle).toBe(E_Orientation.down);	// 205° orients down
	});
