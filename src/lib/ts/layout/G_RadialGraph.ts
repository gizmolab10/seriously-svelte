import { k, u, radial, Ancestry, Predicate, E_Kinship } from '../common/Global_Imports';
	...
	private layout_forPoints_toChildren(points_toChildren: boolean) {
		...
		if (!!focus_ancestry) {
			if (points_toChildren) {
				let childAncestries = focus_ancestry.ancestries_createUnique_byKinship(E_Kinship.child);
