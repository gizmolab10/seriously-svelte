<script lang='ts'>
	...
	const { w_show_show_details_ofType } = show;
	...
	$: { 
		const _ = $w_show_show_details_ofType;
		update_hideable_isVisible();
	}
	...
	function update_hideable_isVisible() {
		...
		if (s_banner_hideable?.hasBanner) {	// d_header has no banner
			isVisible = $w_show_show_details_ofType?.includes(T_Detail[t_detail]) ?? false;
		}
