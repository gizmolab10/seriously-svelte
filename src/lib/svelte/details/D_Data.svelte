<script lang='ts'>
	...
	import { T_Layer, T_Hoverable, T_Request, T_Preference } from '../../ts/common/Global_Imports';
	...
	const s_save = elements.s_element_for(new Identifiable('save'), T_Hoverable.button, 'save');
	...
	function setup_s_elements() {
		...
		for (const id of ids) {
			const s_storage = elements.s_element_for(null, T_Hoverable.database, id);
