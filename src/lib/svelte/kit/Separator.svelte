<script lang='ts'>
	...
	export let top = 0;
	export let height = 0;
	const line_left = isHorizontal ? left + margin : left - thickness / 2;
	...
	// Compute style and Gull_Wings props for both orientations
	$: separatorStyle = isHorizontal
		? `top:${top}px; z-index:${zindex}; position:absolute; left:${line_left}px; height:${thickness}px; width:${width - margin * 2}px; background-color:${separator_color};`
		: `left:${line_left}px; z-index:${zindex}; position:absolute; top:${top + margin}px; width:${thickness}px; height:${height ? height : width - margin * 2}px; background-color:${separator_color};`;

	$: wingsCenter = isHorizontal
		? new Point(width, thickness / 2)
		: new Point(thickness / 2, thickness / 2);

	$: wingsDirection = isHorizontal ? Direction.left : Direction.down;
