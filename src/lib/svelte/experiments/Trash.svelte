<script lang='ts'>
	...
	const viewBox = `0 -0.5 ${k.size.button} ${k.size.button}`;
	const bottom = 12.5;
	const right = 5.6;
	const left = 10.4;
	let top = 6;
...
<div class='trash-container'>
	<svg class='trash-svg' width={k.size.button} height={k.size.button} fill={invert ? color : fill} stroke={color} viewBox={viewBox}>
	...
	<svg class='trash-lines-svg' width={k.size.button} height={k.size.button} fill={invert ? fill : color} stroke={invert ? fill : color} viewBox={viewBox}>
