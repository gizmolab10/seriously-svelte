<script>
	import { k, noop, Thing, Point, ZIndex, dbDispatch, editorGraph, Direction, FatTrianglePath } from "../../ts/common/GlobalImports";
	import { dotDiameter } from '../../ts/managers/State';
	export let origin = new Point($dotDiameter, 20);
	export let direction = Direction.left;
	export let thing;

	const triangle = new FatTrianglePath($dotDiameter + 2, direction);
	...
	function updateColors(isFilled) {
		fillColor = thing.revealColor(isFilled);
	}
