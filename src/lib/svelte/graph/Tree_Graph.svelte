<script lang='ts'>
	...
	const { w_depth_limit, w_scale_factor, w_rect_ofGraphView, w_user_graph_offset } = layout;
	const { w_ancestry_focus } = s;
	let s_component: S_Component;
	let reattachments = 0;

	s_component = signals.handle_anySignal_atPriority(3, $w_ancestry_focus, T_Hoverable.tree, (t_signal, value): S_Component | null => {
		reattachments++;
	});
